...
public class Triomino extends Partie {
	...
	public Triomino() {
		...
		// int [][] this.coupsPossibles=new int[58][2](); //a chaque fois qu on rajoute
		// un trio, on rajoute une possibilité de plus par rapport aux trois cotés
		// initiaux.
		// coupsPossibles represente les coordonnees du plateau ou il est possible de
		// jouer
	...
	public int verif_coll_1_piece(int[] A, int[] aPlacer, int coteCommun) {
		...
		for (int i = 0; i < 3; i++) {
			/*
			 * if(cote!=0)
			 * System.out.println("97 i="+i+" cote="+cote+" c+i="+(cote+i)%3+" a[0]="+A[0]
			 * +" ap[c+i]="+aPlacer[(cote+i)%3]+" a[C]="+A[cote]+" ap[i]="+aPlacer[i]); else
			 * System.out.println("99 i="+i+" cote="+cote+" c+i="+(cote+i)%3+" a[1]="+A[1]
			 * +" ap[2-i]="+aPlacer[2-i]+" a[2]="+A[2]+" ap[(2*i+1)%3]="+aPlacer[(2*i+1)%3])
			 * ;
			 */
			if (cote != 0) {
				if ((A[0] == aPlacer[(cote + i) % 3]) && (A[cote] == aPlacer[i]))// verifie A0 avec X2 (ou X1 a l
																					// envers) et A2 (ou A1 a l envers)
																					// avec X0, puis on les incremente d
																					// 'un pour tester toutes les
																					// positions de X
				{
				...
				if ((A[1] == aPlacer[(2 - i)]) && (A[2] == aPlacer[(2 * i + 1) % 3]))// verifie A1 avec X2 et A2 avec
																						// X1, puis on tourne le trio X
				{
					tourne = i;
					break;
				}
	...
	public void placer_pion(int indice) {
		...
		if (this.nbreCoupsVoulus > 0)// sinon on ne peut rien placer
		{
			...
			{
				System.out.println("num=" + i + " x=" + this.coupsVoulus[i][0] + " y=" + this.coupsVoulus[i][1]);// pzs
																													// de
																													// aff_cp
																													// a
																													// cause
			...
			int pivot = retourCoupPossible[rep + 1];// on regarde quelle rotation on doit faire en fonction de la
			...
			/*
			 * System.out.println("297"); this.aff_cp();
			 */
			...
			/*
			 * System.out.println("305"); this.aff_cp(); this.init_coupsVoulus();
			 * this.aff_cp();
			 */
		...
		for (int i = 0; i < 6; i++) {
			for (int j = i; j < 6; j++) {
				for (int k = j; k < 6; k++) {
					...
					this.tableau[count].set_occupation(true);
					count = count + 1;
				}
			}
		}
	...
	int partie_finie(Joueur[] joueur) {// retourne 0 si la partie doit continuer, numero du joueur+1 si la partie est
	...
	int[] coup_possible(int indice) { // retourne un tableau avec nbreDeCollisions en premiere case et nbreDeTours
										// ensuite, selon la case dans laquelle on va jouer
		int[] aPlacer = new int[3];
		int coll = -1;
		int[] tourne = new int[this.nbrePositionsPossibles + 1];// vaut -1 si le coup n est oas possible, et 0, 1 ou 2
																// selon combien de fois faut faire tourner le trio pour
																// qu'il rentre
		// la premiere case contient la valeur de coll, la suite le nbre de tour que la
		// piece doit faire. ce tableau sera surement trop grand, mais on ne connait pas
		// sa taille exacte a ce moment, donc on a pris sa taille maximale possible
		int retourTourne = -2;
		...
		for (int i = 0; i < this.nbrePositionsPossibles; i++) {
			...
			else if ((valA[0] != -1) && (valB[0] != -1) && (valC[0] != -1))// cette case est entouree de triominos
			{
				...
				if (retourTourne == -1)// la piece n allait pas
					...
					coll = 3;
					tourne[this.nbreCoupsVoulus + 1] = retourTourne;// on met dans la premiere case libre. comme un seul
				...
				val = "A";
				retourTourne = verif_coll_1_piece(valA, aPlacer, cote);// on envoie le cote correspondant, il est
			...
			} else if ((this.coupsPossibles[0][0] == 56) && (this.coupsPossibles[0][1] == 56))// c est le premier coup à
																								// jouer
			{
	...
	public int[][] get_coupsPossibles()
	{
		return this.coupsPossibles;
	}
	public int[][] get_coupsVoulus()
	{
		return this.coupsVoulus;
	}
}